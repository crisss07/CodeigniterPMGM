...
class Predios extends CI_Controller {
	...
	public function form_fusion($id_predio){
		echo "El id predio es:",$id_predio;
		$this->load->view('admin/header');
		$this->load->view('admin/menu');
		// $this->load->view('predios/nuevo', $data);
	...
	public function listar_predio(){
		$url="http://localhost/CodeigniterPMGM/";
        ...
        foreach($predio_consulta as $rows)
        {
            $data[]= array(
				...
				'
					...
					<a href="'. $url .'Reporteseicu/certificacion/'. $rows->predio_id.'" class="btn btn-secondary  fas fa-certificate" aria-hidden="true" title="Certificacion tecnica"> </a>			 
