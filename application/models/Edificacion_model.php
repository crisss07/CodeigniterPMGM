...
class Edificacion_model extends CI_Model {
    ...
    function get_Bloque($predio_id) {//obtiene los datos de la tabla bloque en array result
        $query = $this->db->query("SELECT y.bloque_id,y.predio_id,y.nro_bloque,y.nom_bloque,y.estado_id,e.descripcion,y.altura,y.anio_cons,y.anio_remo,y.porcentaje_remo,y.destino_bloque_id,z.descripcion as desc_bloque_dest,y.uso_bloque_id,x.descripcion as desc_bloque_uso 
FROM catastro.bloque as y LEFT JOIN catastro.uso_bloque as x on x.uso_bloque_id=y.uso_bloque_id LEFT JOIN catastro.destino_bloque as z on z.destino_bloque_id=y.destino_bloque_id
LEFT JOIN catastro.estado e on e.estado_id=y.estado_id
WHERE y.activo=1 and x.activo=1 and z.activo=1 and y.predio_id='$predio_id'");
        return $query->result();
    }
...
WHERE y.activo=1 and x.activo=1 and z.activo=1 and bloque_id='$id'");
   ...
   function get_estado()
    {   
        ...
        $data=$this->db->get_where('catastro.estado',array('activo' => 1 ))->result();
        return $data;
    }
